...
My computer (with a RTX 3060 ti) seems to run best on a policy size of `[1024, 1024, 1024, 1024, 512]` and a critic size of `[2048, 1024, 1024, 1024, 512]`. More than that, and my SPS tanks. If you aren't using a GPU, your CPU is going to take ages to run a large network, so you might need to stick with a smaller one.
